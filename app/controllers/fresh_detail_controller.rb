class FreshDetailController < ApplicationController
	def index
		...
		if params[:code].to_s.length == 4
			#按鲜烟品种统计
			@by_breed = FreshTobacco.find_by_sql ['select f.breed,sum(cast(average_weight as float) * packing_amount/1000) as weight_sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.breed',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟部位统计
		  	@by_part = FreshTobacco.find_by_sql ['select f.part,sum(cast(average_weight as float) * packing_amount/1000) as weight_sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.part',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟成熟度统计
		  	@by_maturity = FreshTobacco.find_by_sql ['select sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_immature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_immature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_mature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_over_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_over_mature from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ?',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟类型统计
		  	@by_type = FreshTobacco.find_by_sql ['select f.tobacco_type,sum(cast(average_weight as float) * packing_amount/1000) as weight_sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.tobacco_type',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	
		  	#按鲜烟品种、成熟度
		  	@by_breed_maturity = FreshTobacco.find_by_sql ['select f.breed,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_immature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_immature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_mature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_over_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_over_mature from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.breed',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟品种、类型
		  	@by_breed_type = FreshTobacco.find_by_sql ['select f.breed,f.tobacco_type,sum(cast(average_weight as float) * packing_amount/1000) as sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.breed,f.tobacco_type',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟品种、部位
		  	@by_breed_part = FreshTobacco.find_by_sql ['select f.breed,f.part,sum(cast(average_weight as float) * packing_amount/1000) as sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.breed,f.part',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟部位、类型
		  	@by_part_type = FreshTobacco.find_by_sql ['select f.part,f.tobacco_type,sum(cast(average_weight as float) * packing_amount/1000) as sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.part,f.tobacco_type',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟部位、成熟度
		  	@by_part_maturity = FreshTobacco.find_by_sql ['select f.part,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_immature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_immature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_mature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_over_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_over_mature from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? and p.created_at >= ? group by f.part',params[:code].to_s,now_time.year.to_s+'-01-01']
			...
			#按鲜烟品种统计
		  	...
		  	@by_breed_maturity = FreshTobacco.find_by_sql ['select f.breed,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_immature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_immature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_mature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_over_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_over_mature from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id where c.code = ? and p.created_at >= ? group by f.breed',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟品种、类型
		  	@by_breed_type = FreshTobacco.find_by_sql ['select f.breed,f.tobacco_type,sum(cast(average_weight as float) * packing_amount/1000) as sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id where c.code = ? p.created_at >= ? group by f.breed,f.tobacco_type',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟品种、部位
		  	@by_breed_part = FreshTobacco.find_by_sql ['select f.breed,f.part,sum(cast(average_weight as float) * packing_amount/1000) as sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id where c.code = ? p.created_at >= ? group by f.breed,f.part',params[:code].to_s,now_time.year.to_s+'-01-01']
		  	#按鲜烟部位、类型
		  	@by_part_type = FreshTobacco.find_by_sql ['select f.part,f.tobacco_type,sum(cast(average_weight as float) * packing_amount/1000) as sum from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id where c.code = ? p.created_at >= ? group by f.part,f.tobacco_type',params[:code].to_s,now_time.year.to_s+'-01-01']
			...
			if params[:type] == "maturity"
				if params[:room_no] == "0"
					@fresh_sum = FreshTobacco.find_by_sql ['select r.room_no,sum(cast(average_weight as float) * packing_amount/1000) from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id where s.code = ? group by r.room_no order by cast(r.room_no as int)',params[:station_code]]
					@maturity = FreshTobacco.find_by_sql ['select r.room_no,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_immature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_immature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_mature,sum((cast(p.average_weight as float) * p.packing_amount/1000) * (f.weight_of_over_mature/(f.weight_of_immature + f.weight_of_mature + f.weight_of_over_mature))) as weight_of_over_mature from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id where s.code = ? group by r.room_no order by cast(r.room_no as int)',params[:station_code]]
					respond_to do |format|
					   format.html
				...
				if params[:room_no] == "0"
					@type = FreshTobacco.find_by_sql ['select r.room_no,f.tobacco_type,sum(cast(average_weight as float) * packing_amount/1000) from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id where s.code = ? group by r.room_no,f.tobacco_type order by cast(r.room_no as int)',params[:station_code]]
					respond_to do |format|
					   format.html
				...
				if params[:room_no] == "0"
					@water_content = FreshTobacco.find_by_sql ['select r.room_no,f.water_content,sum(cast(average_weight as float) * packing_amount/1000) from packings p left join fresh_tobaccos f on f.task_id = p.task_id left join tasks t on t.id = p.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id where s.code = ? group by r.room_no,f.water_content order by cast(r.room_no as int)',params[:station_code]]
					respond_to do |format|
					   format.html
