class DryDetailController < ApplicationController
	def index
		if params[:code].to_s.length == 4
		    ...
		    @by_breed_part = DryTobacco.find_by_sql ["select trim(f.breed) as breed,trim(f.part) as breed,sum(d.weight * p.packing_amount) as sum from dry_tobaccos d left join packings p on p.task_id = d.task_id left join fresh_tobaccos f on f.task_id = d.task_id left join tasks t on t.id = d.task_id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id left join counties c on c.id = s.county_id left join cities ci on ci.id = c.city_id where ci.code = ? group by trim(f.breed),trim(f.part)",params[:code].to_s]
			...
			if params[:type] == "sum"
				if params[:room_no] == "0"
					@dry_sum = DryTobacco.find_by_sql ['select r.room_no,c.party_b,c.work_started,c.work_finished,f.breed,sum(d.weight * p.packing_amount) as sum from dry_tobaccos d left join packings p on p.task_id = d.task_id left join fresh_tobaccos f on f.task_id = d.task_id left join tasks t on t.id = d.task_id left join contracts c on c.task_id = t.id left join rooms r on r.id = t.room_id left join stations s on s.id = r.station_id where s.code = ? group by r.room_no,f.breed,c.party_b,c.work_started,c.work_finished order by cast(r.room_no as int),c.work_started',params[:station_code]]
					respond_to do |format|
					   format.html
